...
function gwolle_gb_maybe_encode_emoji( $string, $field ) {
	...
	if ( method_exists($wpdb, 'get_col_charset') ) {
		...
		if ( 'utf8mb4' === $charset ) {
			// make sure we support misconfigured hosts.
			$wpdb->set_charset( $wpdb->dbh, 'utf8mb4' );
		} else if ( 'utf8mb4' != $charset && function_exists('wp_encode_emoji') && function_exists( 'mb_convert_encoding' ) ) {
			// we don't support the proper charset, so encode to html entities.
