			...
			if ( isset($foundTables[0]) && in_array( $wpdb->prefix . 'dmsguestbook', $foundTables[0] ) ) {
				...
				if ( is_array($result) && !empty($result) ) {
					...
					if ( $saved == 0 ) {
						...
						$gwolle_gb_messages .= '<p>' . sprintf( _n('%s entry imported successfully from DMSGuestbook.','%s entries imported successfully from DMSGuestbook.', $saved, 'gwolle-gb'), $saved ) . '</p>';
					}
				} else {
					$gwolle_gb_errors = 'error';
			...
			if ( is_array($args) && !empty($args) ) {
				...
				if ( is_array($comments) && !empty($comments) ) {
					...
					if ( $saved == 0 ) {
						...
						$gwolle_gb_messages .= '<p>' . sprintf( _n('%s entry imported successfully from WordPress comments.','%s entries imported successfully from WordPress comments.', $saved, 'gwolle-gb'), $saved ) . '</p>';
					}
				} else {
					$gwolle_gb_errors = 'error';
					...
					if( $_FILES['start_import_gwolle_file']['size'] > (1024000) ) { //can't be larger than 1 MB
						...
						if ( !in_array( $mimetype, $mimetypes ) ) {
							...
							if ( $row == 0 ) {
								...
								$gwolle_gb_messages .= '<p>' . sprintf( _n('%s entry imported successfully from the CSV file.','%s entries imported successfully from the CSV file.', $row, 'gwolle-gb'), $row ) . '</p>';
							}
