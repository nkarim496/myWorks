<div class="box">
      ...
      <li><span id="filter-group<?php echo $filter_group['filter_group_id']; ?>"><?php echo $filter_group['name']; ?></span>
        <ul>
          ...
          <li>
			...
			<label for="filter<?php echo $filter['filter_id']; ?>"></label>
 ...
 <!--   <input type="button" id="button-filter" class="button" value="<?php echo $button_filter; ?>" /> -->
  </div>
</div>
<script type="text/javascript"><!--
$('.box-filter').bind('click', function() {
	filter = [];
	...
	//location_filter = '<?php echo $action; ?>&filter=' + filter.join(',');
	//$('.product-grid').load(location_filter + ' .product-grid > *');
});
//--></script> 
